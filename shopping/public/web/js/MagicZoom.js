...
function MagicZoom_findZooms() {
    ...
    for (var i = 0; i < aels.length; i++) {
        if (/MagicZoom/.test(aels[i].className)) {
            ...
            var opacity = 50;
            if (matches) {
                opacity = parseInt(matches[4])
            }
            re = new RegExp(/thumb\-change(\s+)?:(\s+)?(click|mouseover)/i);
            matches = re.exec(aels[i].rel);
            var thumb_change = 'click';
            if (matches) {
                thumb_change = matches[4]
            }
            re = new RegExp(/zoom\-width(\s+)?:(\s+)?(\w+)/i);
            matches = re.exec(aels[i].rel);
            bigCont.style.width = '400px';
            if (matches) {
                bigCont.style.width = matches[4]
            }
            re = new RegExp(/zoom\-height(\s+)?:(\s+)?(\w+)/i);
            matches = re.exec(aels[i].rel);
            bigCont.style.height = '400px';
            if (matches) {
                bigCont.style.height = matches[4]
            }
            ...
            var drag_mode = false;
            if (matches) {
                if (matches[4] == 'true') drag_mode = true
            }
            re = new RegExp(/always\-show\-zoom(\s+)?:(\s+)?(true|false)/i);
            matches = re.exec(aels[i].rel);
            var bigImage_always_visible = false;
            if (matches) {
                if (matches[4] == 'true') bigImage_always_visible = true
            }
